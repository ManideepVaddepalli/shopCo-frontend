...
export default function ActualProduct({ props }) {
  const fullStar = (
    <svg
      className="rating-star-product"
      width="34"
      height="32"
      viewBox="0 0 34 32"
      fill="none"
  ...
  let stars = ratingStars(props.avgRating);
  return (
    <div className="total-actual-product-container">
      <ImagesComponent
        displayImage={props.displayImage}
        image1={props.images[0]}
        image2={props.images[1]}
        image3={props.images[2]}
      />
      <div className={`${satoshi.className} text-total-container-product`}>
        <h1 className={integralCFBold.className}>{props.name}</h1>
        <div className="rating-container-product">
          <div className="stars-container-product">{stars}</div>
          <p>
            {props.avgRating}/<span>5</span>
          </p>
        </div>
        <h4 className="price-container-product">
          <span className="discount-price-product">
            ₹.{props.discount ? props.discountPrice : props.actualPrice}
          </span>
          {props.discount ? (
            <span className="actual-price-product">₹.{props.actualPrice}</span>
          ) : (
            <></>
          )}
          {props.discount ? (
            <span className="discount-product">-{props.discount}%</span>
          ) : (
            <></>
          )}
        </h4>
        <p className="description-product-container">{props.description}</p>
        <div className="color-container-product">
          <p>Color</p>
          <div className="individual-colors-container-product">
            {props.colors.map((elem) => {
              return (
                <div
                  key={elem}
                  className="color-div-product"
                  style={{ backgroundColor: elem.toLowerCase() }}
                >
                  <p>{elem[0].toUpperCase() + elem.slice(1).toLowerCase()}</p>
                </div>
              );
            })}
          </div>
        </div>
        <div className="choose-size-container-product">
          <p>Choose Size</p>
          <div className="all-sizes-container-product">
            {props.sizes.map((elem) => {
              return (
                <div key={elem} className="single-size-container-product">
                  <input
                    name="size"
                    value={elem}
                    className="common-input-size-product"
                    type="radio"
                  ></input>
                  <div className="one-sizename-container-product">{elem}</div>
                </div>
              );
            })}
          </div>
