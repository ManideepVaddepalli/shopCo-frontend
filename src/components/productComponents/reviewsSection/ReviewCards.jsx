...
export default function ReviewCards({ props }) {
  const fullStar = (
    <svg
      className="star-review-card-product"
      width="34"
      height="32"
      viewBox="0 0 34 32"
      fill="none"
  ...
  let cardElements = props.map((elem) => {
    let stars = ratingStars(elem.rating);
    let date = new Date(elem.postedOn);
    const postMonth = months[date.getMonth()];
    const postDate = date.getDate();
    const postYear = date.getFullYear();
    return (
      <div key={elem._id} className="single-card-container-review-product">
        <div className="stars-container-review-card">{stars}</div>
        <div className="name-greencircle-review-card-product">
          <h4>{elem.reviewName}</h4>
          <div className="user-verified-product-review"></div>
        </div>
        <p className="para-product-review-card">{elem.reviewDesc}</p>
        <p className="posted-on-product-review-card">
          Posted on {postMonth} {postDate}, {postYear}
        </p>
      </div>
    );
  });
  return (
    <div className={`${satoshi.className} all-cards-container-review-product`}>
      {cardElements}
    </div>
  );
}
